    ...
    try:
        ...
        for column in content[read_from_index:]:
            if column.strip().startswith("from"):
                ...
                logger.info(
                    "Finish with explicit 'final as(' statement or a flat select"
                )
                logger.info(
                    "The final version requires the line: 'select * from final\\n'"
                )
                exit()
            if column.count("--") > 0:
        ...
        logger.info(
            "i.e. use the with clause and 'final as(', or flat select statements"
        )
        logger.info(e)
        exit()
...
def update_yml_dict(*, yml_dict: dict, sql_dict: dict, yml_file: str) -> None:
    ...
    for model in yml_dict["models"]:
        ...
        new_model_cols = [
            col for col in model_cols if col["name"] in sql_dict[model_name]
        ]
...
def update_yamls_from_sqls_in_dir(files_and_dirs: list, dir_path: str = None) -> None:
    ...
    if yml_file:
        ...
        if yml_dict:
            ...
            if "models" not in yml_dict or not yml_dict["models"]:
                logger.info(
                    f"No 'models' found in {yml_file[0]}. Please delete the file and rerun this program."
                )
                return
            else:
                logger.info(
                    f"models section found in {yml_file[0]} file, updating column names with .sql files"
                )
                update_yml_dict(
                    yml_dict=yml_dict, sql_dict=sql_dict, yml_file=yml_file[0]
                )
            ...
            logger.info(
                f"The YAML file {yml_file[0]} is empty or invalid. Deleting the file."
            )
    ...
    if len(yml_file) == 0 and len(sql_files) > 0:
        ...
        yml_dict = {
            "version": "2",
            "models": [{"name": "dummy", "columns": [{"name": "aarmnd"}]}],
        }
...
def generate_comments_from_sql(
    *, models_path="dbt_utsikt/models", docs_path="dbt_utsikt/docs"
) -> None:
    def find_project_root(current_path):
        while (
            current_path != current_path.parent
        ):  # Continue until reaching the root of the filesystem
    ...
    default_content = {"custom_column_comments": {}, "custom_table_descriptions": {}}
    ...
    if file_path.exists():
        ...
        try:
            ...
            logger.info(
                f"File '{file_path}' created with default content. Remember to fill relevant information in this file and rerun the current program."
            )
        except Exception as e:
            logger.info(e)
    ...
    try:
        ...
        logger.info(e)
        logger.info(
            "Fant ikke 'comments_source.yml' fil som inneholder kommentarer fra source"
        )
        ...
        content = (
            ...
            '    sources_yml_path="dbt_utsikt/models/sources.yml",\n'
            ")"
        )
        try:
            if file_path.exists():
                ...
                logger.info(
                    'Please execute "get_comments_source.py" file first, and then re-run the current program.'
                )
            else:
                ...
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(content)
                logger.info(
                    f"The file '{file_path}' has been created with the necessary instructions."
                )
                logger.info(
                    "Please execute this file first, and then re-run the current program."
                )
            ...
            for t in tabeller:
                ...
                for c in t_columns:
                    ...
                    try:
                        ...
                        logger.info(
                            f"{c_name} har ikke felt for beskrivelse i {t_name}"
                        )
                        continue
